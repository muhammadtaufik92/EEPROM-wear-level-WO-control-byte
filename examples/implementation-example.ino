...
#define IDX_VAR_NTH    (uint8_t[]){IDX_VAR_A, IDX_VAR_B, IDX_VAR_C}
  ...
  if(Serial.available() > 0){
    ...
    if(str[0]=0x21){
      if (str[1]==IDX_VAR_A) {
        ...
        Serial.println("VAR "+IDX_VAR_A+"is successfully updated to "+EEPROMwl.read(IDX_VAR_A, address));
        cmd_available=1;
      }
      if (str[1]==IDX_VAR_B) {
        EEPROMwl.update(IDX_VAR_B, (uint8_t)str[3]);
        Serial.println("VAR "+IDX_VAR_B+"is successfully updated to "+EEPROMwl.read(IDX_VAR_B, address));
        cmd_available=1;
      }
      if (str[1]==IDX_VAR_C) {
        EEPROMwl.update(IDX_VAR_C, (uint8_t)str[3]);
        Serial.println("VAR "+IDX_VAR_C+"is successfully updated to "+EEPROMwl.read(IDX_VAR_C, address));
        cmd_available=1;
      }
      if (!cmd_available) {
